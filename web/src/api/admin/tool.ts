...
// create/update/preview/refresh 都可能在服务端现场连远端 MCP 做发现（预算 ≈45s > axios 默认 30s），
// 不带专用超时会客户端先断报「网络异常」（T4b 验收实测；同 llmTestTimeoutMs 教训）。
const DISCOVER_CFG = { timeout: config.mcpDiscoverTimeoutMs }
...
export function createTool(body: ToolUpsertBody) {
  return request.post<ToolAdminItem>(BASE, body, DISCOVER_CFG)
}

/** 全量更新（头值留空=不改；mcp 行须带 type:'mcp'）。后端：PUT .../{id} */
export function updateTool(id: string, body: ToolUpsertBody) {
  return request.put<ToolAdminItem>(`${BASE}/${id}`, body, DISCOVER_CFG)
}
...
export function previewTool(body: ToolPreviewBody) {
  return request.post<ToolPreview>(`${BASE}/preview`, body, DISCOVER_CFG)
}

/** 重新发现 MCP 工具清单（仅 mcp 行；凭据用库中密文）。后端：POST .../{id}/refresh */
export function refreshTool(id: string) {
  return request.post<ToolAdminItem>(`${BASE}/${id}/refresh`, undefined, DISCOVER_CFG)
}
