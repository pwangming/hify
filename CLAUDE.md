...
> 二期可考虑：混合检索+Rerank（PG 的 tsvector 全文检索 + pgvector，地基已就绪）、DSL导入导出、Embedding结果缓存、多副本扩容时引入 Redis 作共享缓存、工作流注册为 Agent 工具（经 tool 模块绕行，禁止 conversation→workflow 直接依赖）

### 技术栈
**后端**：Spring Boot 3.x（Java 21 虚拟线程）+ Spring AI（多模型ChatClient/Tool Calling/RAG Advisors/VectorStore）+ Spring Modulith（模块边界校验）+ MyBatis-Plus + PostgreSQL 16 + pgvector（业务数据与向量一库通吃）+ Caffeine 进程内缓存（一期不引入 Redis）+ Resilience4j（LLM 调用韧性）

**前端**：Vue 3 + TypeScript + Element Plus（管理后台/表单）+ Vue Flow（Workflow画布）
...
├── web/                 # Vue 3 前端（独立 npm 工程，同仓库；构建产物进 nginx 镜像）
