...
**Workflow引擎**：可视化画布，支持5类节点 —— LLM、知识检索、HTTP请求、代码执行(Python沙箱)、条件分支
（另有 start/end 为引擎内建）。**不含 "Agent调用" 节点**——早期规划写过，2026-07-19 复盘时决定不做：
LLM 节点已覆盖编排里"调一次模型"的需求，而"让编排中的某一步自主循环调工具"对单团队内部场景
价值不明确，不值得为它引入 workflow→agent 的模块依赖。需要 Agent 能力时直接用对话应用的 Agent 模式。
...
├── mcp-demo/            # 自建 MCP server 练手工程 + E2E 测试夹具（TypeScript，独立 pnpm 工程，端口 3100）
│                        # 真实自建 MCP server 一律单开仓库，不进本仓库（deployment.md §5）
...
| `data-model.md` | 19 张表清单与关系、跨模块不建外键、刻意不存在的表 | 涉及新表/改表结构 |
