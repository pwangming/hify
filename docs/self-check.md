...
>
> 全文按**轮次时间序**排列，一轮一节（`##`）。早期 provider C2 / conversation ③④⑤ / usage ⑥
> 四轮曾各自单开 `docs/self-check-*.md`，2026-07-21 已并入本文件，旧文件删除——
> 历史 plan/spec 里指向那些文件名的链接，都到本文件对应小节里找。
> 手工接口验收（Postman）见 `docs/postman/`，E2E 操作手册见 `docs/e2e-guide.md`。
...
## 2026-06-26 provider C2：getChatClient + 四层韧性
...
**状态：✅ 已验证（用户 2026-06-26 手动完成）**

用户重建真实供应商 + API Key + 绑定模型的对话型应用后，在「试聊」页发送消息，确认收到真实模型回复、链路打通（identity 登录 → 应用绑定模型 → provider 建 ChatClient → conversation 组 prompt 一次性 .call → 落库 → 前端渲染）。至此本轮唯一需活模型才能证明的部分（自动化测试以 mock 替过）已闭环。

> 单轮边界确认：同一会话不带记忆（prompt 只喂当前消息），多轮记忆为下一轮范围。
...
## 2026-06-29 conversation ⑤ SSE 流式输出
...
## 2026-07-18 用量与成本看板轮：终审 + 验收排障自检（本轮收官）

- 范围：spec/plan（0eb649c/b76212f）→ Codex 执行 Task 1-10（79de2fd..de4b8c3）→ Claude 终审 → 验收排障三轮（b2acf03..8ae19b5）→ 用户验收通过。
- 终审结论：V26 与计划逐字一致（diff 空）；TokenUsedEvent 全库 6 构造点无漏；白名单三件套（package-info/code-organization 表/ModularityTests）齐；偏差 12 处全部入档且合理，其中 2 处是对计划真实 bug 的正确修复（verifyNoMoreInteractions 顺序、Vitest 4 vi.hoisted）；LayerRulesTest 加 `..mapper..` 自身豁免属合理最小修复（mapper 内嵌 SQL 投影 record 与外层接口互依赖）。
- 终审亲跑回归：`mvn verify` 退出码 0（716/0/0，4 个连库 Test + 两架构测试实证跑进 verify）；前端 410 passed + typecheck 0。
- **验收抓出两个跨轮潜伏计量黑洞**（均非本轮引入，看板首次曝光；证据链：库中最新流水停在 07-16 且 source 全空 vs message 181/run 46 当天成功，backend.log 与 pg 日志双零错误）：
  1. `appendAssistant` 8 参重载缺 `@Transactional` 且 `this.` 委托绕过代理 → 非 Agent 聊天（同步+SSE）发计量事件时无事务 → AFTER_COMMIT **静默丢弃**。自 Agent 轮引入重载起漏记用量与配额。修复：8 参重载补 `@Transactional`（b2acf03）。
  2. workflow 引擎禁事务（铁律），`LlmNodeExecutor` 发布点天然无事务 → 同机制静默丢弃，**W1 起工作流 LLM 用量从未记账**。修复：监听器 `fallbackExecution=true`（计费语义：LLM 返回即 token 已消耗，不依赖后续事务；有事务时仍 AFTER_COMMIT）。
  - 两修复均先写「真提交事务」连库测试拿红灯再改（UsageEventFlowDbTest / ConversationUsageFlowDbTest），顺带补上「AFTER_COMMIT 事件链从未被集成测试覆盖」盲区（旧连库测试全回滚事务，该链此前仅靠手动验收）。修复后线上实证：流水 57=conversation、58=workflow（W1 以来第一条工作流用量）。
  - 历史漏记数据不可追回（事件当时已丢），旧日期的工作流/非 Agent 用量缺失属预期；历史流水 source=NULL 显示「—」，来源筛选不含其为正确行为。
- 验收 UI 修缮（1fb6609/f0b13ea/8ae19b5）：菜单图标（iconMap 未登记 DataAnalysis）、趋势图图例挪底部（TDD）、排行 tab 改 v-model+@tab-change（原 label 内 span 绑点击，点 tab 头 padding 无反应）、调用日志默认窗口上界取整当天 23:59:59（原冻结在挂载瞬间致新记录查不到，TDD）+ 下拉定宽 + 返回看板按钮（TDD）、日期组件宽度三连修——内联 style 被透传吃掉 → :deep 260px 仍被未知全宽规则压制（构建产物/dev server/EP 级联四层取证均正确，风源未定位）→ 终以 width+max-width+flex-grow 三保险 !important 收窄，用户确认生效。
- 最终回归：后端 719/0/0（716+3 新增）退出码 0；前端 410 passed、typecheck 0。
- 留账：① 日期组件全宽拉伸的风源未定位（!important 补丁掩盖，下次动全局样式时留意）；② daily_usage 与 usage_stat_daily 双聚合表冗余，二期配额切新表后废弃；③ 耗时/成败观测列二期。
...
- 2026-07-18 workflow 旅程变异 DoD：删掉 `kb_1→llm_1` 连线后，运行阶段 `run-chip` 成功断言红（元素不存在）；将 `STUB_VEC` 改为全零后，节点运行输出为 `count: 0`，步骤 15 的命中断言红。两次变异均恢复源码后完整 workflow 旅程重新 `1 passed`。
- 2026-07-18 Agent 旅程变异④（终审补证，Codex 勾选但未留实录）：临时注释 playwright.config.ts 的 mcp-demo webServer 条目后重跑 agent 旅程，红在预期断言 `agent-journey.spec.ts:40`（`mcp-tool-get_current_time` 发现失败，MCP 互通不是假的）；还原后单跑 agent 旅程 `1 passed`，工作区与提交态 diff 为空。
