...
- [ ] **Step 1: 新建 `components/ToolDrawer.vue`**（表单逻辑自 ToolList 原样搬入，类名前缀 `tool-list__` → `tool-drawer__`）
...
- [ ] **Step 2: 重写 `ToolList.vue`**（列表逻辑保留，抽屉换成组件引用）
...
- [ ] **Step 3: 改 `types/tool.ts`**（`ToolAdminDetail`/`ToolForm`/`ToolPreview` 三个接口整体替换，`ToolPreview` 前插入两个新类型）
...
- [ ] **Step 5: 改 `ToolDrawer.vue`（调用点适配，UI 不变）**
