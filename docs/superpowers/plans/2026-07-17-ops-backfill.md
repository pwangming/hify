...
- [x] **Step 1: 临时调大沙箱超时（override 文件注入，不改任何仓库文件）**
...
- [x] **Step 5: 反向场景——超时强杀 + ZombieRunResetter 衔接**
