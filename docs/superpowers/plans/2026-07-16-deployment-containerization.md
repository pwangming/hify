...
- [x] **Step 1: `web/package.json` 补 packageManager 字段**
...
- [x] **Step 3: 写 `deploy/nginx/nginx.conf`**
...
- [x] **Step 4: 写 `deploy/nginx/Dockerfile`**
...
RUN corepack enable
WORKDIR /build
# pnpm-workspace.yaml 带 allowBuilds 白名单（esbuild 等 postinstall），缺了 pnpm11 会报
# ERR_PNPM_IGNORED_BUILDS 退出非零；.npmrc 带 engine-strict，都须在 install 前就位
...
- [x] **Step 5: 写 `deploy/nginx/gen-self-signed-cert.sh` 并加执行位**
