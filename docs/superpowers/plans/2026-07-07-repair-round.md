...
- [x] **Step 4: 前端提示行（ProviderList.vue）**
...
- [x] **Step 5: 前端回归（静态文案不加测试，靠既有用例不红 + 类型检查）**
...
- [x] **Step 2: 运行确认失败（编译错：StreamEvent.Meta 不存在）**
...
- [x] **Step 3: KbChunkRetrievalTest 归位 mapper 包**
...
- [x] **Step 4: clean 全量回归（必须 clean——旧 class 残留会让 ArchUnit 误报）**
...
- 改了什么：从 `main` 的 `e85636a` 建立 `fix/repair-round` 分支。
- 红灯覆盖点：无代码改动，无测试红灯。
- 绿灯后测试数：未运行测试。
- commit hash：无独立代码提交（建分支步骤已勾选）。

### Task 1: ChatClientFactory 显式路径 + baseUrl 归一化

- 改了什么：`ChatClientFactory` 新增 `normalizeBaseUrl` 与 `buildOpenAiApi`，OpenAI chat/embedding 统一显式使用 `/chat/completions` 与 `/embeddings`；测试反射断言任意版本前缀 baseUrl 装配。
- 红灯覆盖点：`ChatClientFactoryTest` 因 `normalizeBaseUrl` / `buildOpenAiApi` 缺失失败。
- 绿灯后测试数：`ChatClientFactoryTest,ModelConnectionServiceTest,ProviderFacadeImplTest` 共 24 个测试通过。
- commit hash：`a67469a`

### Task 2: V19 迁移 + 文档 §6.1 重写 + 前端提示行

- 改了什么：新增 V19 迁移补存量 openai baseUrl 版本段；重写 `llm-resilience.md` §6.1；ProviderList Base URL 输入增加完整基址示例和提示。
- 红灯覆盖点：连库迁移链真验 V1..V19，覆盖 V19 语法和全量 Flyway 应用；前端靠既有 ProviderList 用例和 build 覆盖表单改动。
- 绿灯后测试数：`AppDatasetRelSchemaTest` 2 个测试通过；`ProviderList.spec.ts` 15 个测试通过；`pnpm build` 通过。
- commit hash：`d7f70fd`

### Task 3: 同步端点失败清理

- 改了什么：`ConversationService.send` 在 openTurn 后的 LLM 获取/调用/落 assistant 任一步失败时调用 `cleanupFailedTurn`，清理失败只记 warn 并透传原异常。
- 红灯覆盖点：3 个同步失败用例因 `cleanupFailedTurn` 未调用失败，覆盖模型不可用、新会话调用失败、续聊调用失败；新增清理失败不吞原始异常。
- 绿灯后测试数：`ConversationServiceTest` 23 个测试通过。
- commit hash：`d343d9f`

### Task 4: SSE 开场 meta 事件（后端）

- 改了什么：新增 `StreamEvent.Meta`、`StreamPayloads.Meta`，`sendStream` 在首个 delta 前发 meta，controller 映射 `event: meta`；原 `message/done/error` 不改。
- 红灯覆盖点：service/controller 测试因 `StreamEvent.Meta` 缺失失败，覆盖成功流、错误流、新会话错误、续聊错误、成功不清理和 SSE 输出。
- 绿灯后测试数：`ConversationServiceTest,ConversationControllerTest` 共 34 个测试通过。
- commit hash：`85b5147`

### Task 5: 前端 meta 处理

- 改了什么：`useChatStream` 增加可选 `onMeta` 并分发 `event: meta`；conversation store 在新会话 meta 到达时立即写 `currentId`，断流重发走续聊。
- 红灯覆盖点：`useChatStream` meta 顺序测试失败为只收到 done；store meta 后断流测试失败为 `currentId` 仍为 null。
- 绿灯后测试数：局部 24 个测试通过；前端全量 31 个文件、231 个测试通过；`pnpm build` 通过。
- commit hash：`7607d86`

### Task 6: ArchUnit 规则归位

- 改了什么：`LayerRulesTest` 使用 `DoNotIncludeTests`；`@Transactional` 回到 `PgIntegrationTest`；删除 `TransactionalPgIntegrationTest`；`KbChunkRetrievalTest` 移到 `knowledge.mapper` 包。
- 红灯覆盖点：本任务按计划直接做测试侧归位；强制 `mvn clean test` 覆盖旧 class 残留、ArchUnit/Modularity 和连库测试。
- 绿灯后测试数：`mvn clean test` 提权重跑后 430 个测试通过；`LayerRulesTest` 5 条规则通过。
- commit hash：`16b4745`

### Task 7: 全量回归 + 手动验收

- 改了什么：执行后端 `mvn clean verify`、前端 `pnpm test`、前端 `pnpm build`；手动验收清单标注为待用户验收。
- 红灯覆盖点：无新增代码红灯；覆盖全量后端测试、打包、前端全量测试、类型检查与生产构建。
- 绿灯后测试数：后端 `mvn clean verify` 430 个测试通过并完成 jar/repackage；前端 31 个文件、231 个测试通过；`pnpm build` 通过。
- commit hash：本执行记录提交（计划未给 Task 7 commit message，详见偏差清单）。
- 手动验收：待用户验收。
