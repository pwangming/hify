...
- [ ] **Step 1: 写失败测试**——`RunStatusChip.spec.ts` 全文（el-popover 本地 stub：reference/default 两 slot 都直出）：
...
- [ ] **Step 3: 实现**——`RunStatusChip.vue` 全文：
...
- [ ] **Step 1: 写失败测试**——`CanvasNode.spec.ts` 追加（沿该文件既有 mount 写法，provide 用 `global.provide`；`NODE_RUNS_KEY` 是 Symbol，provide 键直接用它）：
...
- [ ] **Step 1: 写失败测试**——`NodeRunPanel.spec.ts` 全文：
...
- [ ] **Step 1: 写失败测试**——`NodeConfigDrawer.spec.ts` 追加：
...
- [ ] **Step 3: 实现**——`NodeConfigDrawer.vue`：
