...
- [ ] **Step 2: golden-journey.spec.ts 改用助手**——删掉文件内的 ADMIN/PASS 常量、登录四行、pickOption 函数，换成 `import { login, pickOption, uniqSuffix } from './support/ui'`，`const uniq = uniqSuffix()`，步骤 1) 整段换成 `await login(page)`。其余逐字不动。

- [ ] **Step 3: 写 workflow 旅程上半**
