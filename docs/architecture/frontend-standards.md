...
9. **优先用 Element Plus 组件，不手写已有能力**。凡 Element Plus 提供的交互/容器（折叠 `el-collapse`、滚动容器 `el-scrollbar`、下拉 `el-dropdown`、弹窗 `el-dialog`、表格 `el-table`、菜单 `el-menu`、标签页 `el-tabs` 等），一律用组件库，**禁止自己用 `div`+`v-show`+点击事件重造**。
   - *为什么*：组件库已处理可访问性、键盘操作、动画、边界态，自造的又少又容易出 bug，且风格不统一。
   - **图标同理**：一律用 `@element-plus/icons-vue`（按需 `import { Plus } from '@element-plus/icons-vue'`，套 `<el-icon>` 用），**禁止手写字符三角（▼/▶）、emoji 或自绘 SVG 当图标**。
   - 只有组件库确实没有对应能力时，才自己实现，并在 PR/提交说明里点明"库无此组件"。
